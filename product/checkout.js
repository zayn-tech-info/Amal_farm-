...
carts = carts.map((item) => {
  if (!item.price || !item.kilo || !item.image) {
    const matchingProduct = productsDatabase.find(
...
function renderCartItems() {
  ...
  if (carts.length === 0) {
    cartHTML +=
      '<p style="text-align: center; color: #666;">Your cart is empty. <a href="product.html">Continue shopping</a></p>';
  } else {
      ...
      cartHTML += `
                <div class="cart-item">
                    <div class="item-image">
                        ${
                          itemImage
                            ? `<img src="${itemImage}" alt="${item.productName}" style="width: 100%; height: 100%; object-fit: cover;">`
                            : `<i class="fas fa-fish" style="font-size: 2rem; color: #006D5B;"></i>`
                        }
                    </div>
                    <div class="item-details">
                        ...
                        <div class="item-price">${
                          itemPrice
                            ? formatCurrency(parsePrice(itemPrice))
                            : "Price N/A"
                        }</div>
                        <div class="item-meta">Weight: ${
                          itemKilo ? itemKilo : "N/A"
                        }</div>
                    </div>
...
function updateOrderSummary() {
  ...
  // Update summary
  const orderSummaryItem = orderSummaryContainer.querySelector(
    ".order-summary-item"
  );
  if (orderSummaryItem) {
